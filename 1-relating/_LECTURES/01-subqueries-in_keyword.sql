...
SELECT "name" FROM authors              -- get name from from author id
WHERE "id" = (
    SELECT "author_id" FROM authored    -- get authored_id from book id
    WHERE "book_id" = (
        SELECT "id" FROM books          -- get id from books table based of title
        WHERE "title" = 'Flights'
    ) 
);
